...
Код выхода 0. Состояние — `/var/lib/fleet-guards/traffic.json`: по каждой ноде период (дата последнего сброса счётчика у хостера — `trafficResetDay` ноды в панели, 1-е число, если не задан), накопленные rx/tx, последние значения счётчиков, `started_at`, отправленные пороги. Длинные отчёты уходят в Telegram несколькими сообщениями (лимит 4096 символов); имена нод экранируются.
